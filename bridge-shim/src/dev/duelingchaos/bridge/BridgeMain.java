...
public class BridgeMain {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 2) {
            System.err.println("Usage: BridgeMain <deck1.dck> <deck2.dck> [port] [preconsDir] [aiProfile] [gameMode]");
            System.exit(1);
        }
        ...
        // fresh 60x-basic-land deck plus the Momir Vig avatar (confirmed via
        // javap: GameType.MomirBasic's deckAutoGenerator lambda builds
        // exactly that CardPool), so the deck1/deck2 file args are ignored
        // rather than loaded.
        ...
        // RegisteredPlayer's constructor doesn't populate vanguardAvatars on
        // its own (only Deck's [Avatar] section -> Command zone at game
        // setup does, and only once this list is non-null) — confirmed via
        // javap: assignVanguardAvatar() must be called explicitly to pull it
        // from the deck. A no-op for decks with no [Avatar] section, so safe
        // to call unconditionally rather than gating on `momir`.
